...
class PersonalStore:
    ...
    def _migrate(
        ...
        try:
            cur.execute(
                "ALTER TABLE tasks ADD COLUMN subtasks_json TEXT DEFAULT '[]'"
            )
        except Exception:
            pass
        ...
        try:
            cur.execute(
                "ALTER TABLE tasks ADD COLUMN remind_offsets_json TEXT DEFAULT '[]'"
            )
        except Exception:
            pass
        ...
        try:
            cur.execute(
                "ALTER TABLE users ADD COLUMN password_scheme TEXT NOT NULL DEFAULT 'sha256_legacy'"
            )
        except Exception:
            pass
    ...
    def _verify_password_pbkdf2(
        ...
        calc_hash = self._hash_password_pbkdf2(
            password,
            salt_b64,
            iterations
        )
    ...
    def register_user(
        ...
        password_hash = self._hash_password_pbkdf2(
            password,
            salt,
            iterations
        )
    ...
    def login_user(
        ...
        with self._conn() as con:
            ...
            if user.password_scheme == self.PBKDF2_SCHEME:
            ...
            elif user.password_scheme in (
                None,
                "",
                "sha256_legacy"
            ):

                legacy_hash = self._hash_password_legacy(
                    password
                )
                ...
                new_hash = self._hash_password_pbkdf2(
                    password,
                    new_salt,
                    new_iterations
                )
                ...
                raise ValueError(
                    "Неизвестная схема хранения пароля."
                )
